...
/**
 * Default I2c address of notecard, `0x17`
 */
 ...
 */
export let MAX_REQUEST_QUEUE = 5
...
/**
 * The number of pending requests, default maximum is 5.
 */
 ...
 *
 * The product and serial are read from arguments, or settings (NOTE_PUID, NOTE_SN).
...
export async function init(options?: Partial<HubSetRequest>) {
    ...
    if (!req.product) req.product = await readSetting("NOTE_PUID")
    if (!req.sn)
        req.sn =
            (await readSetting("NOTE_SN")) || "devs:" + deviceIdentifier("self")
    await request(req)
}
